...
<div class="content">
	...
	<table class="tb">
		...
		<tr>
			<td>磁盘空间</td>
			<td>10M+</td>
			<td><?php echo function_exists('disk_free_space') ? get_byte(disk_free_space(ROOT)) : 'unknow'; ?></td>
		</tr>
		<tr>
			<td>PDO扩展</td>
			<td>必须开启</td>
	...
	<table class="tb">
		...
		$dirs = array('/'.APP_NAME.'/config', '/'.APP_NAME.'/log', '/'.APP_NAME.'/runtime', '/'.APP_NAME.'/plugin', '/'.APP_NAME.'/view', '/'.APP_NAME.'/upload');
		foreach($dirs as $dir) {
			$ret = dirWrite(ROOT.$dir, TRUE);
			...
			if(!empty($ret['no'])) {
				...
				foreach($ret['no'] as $i => $row) {
					echo '<br>['.$row[1].'] '.str_replace(ROOT, '', $row[0]);
					if($i>8) {
						echo '<br>******'; break;
					}
				}
